...
class ResellerInvoiceDataFeExportController extends Controller
{
    public function exportRenewalSales($resellerHandoverFeId)
    {
        return $this->exportInvoiceData($resellerHandoverFeId, [108, 109, 110, 111], 'RENEWAL');
    }

    public function exportAddOnSales($resellerHandoverFeId)
    {
        return $this->exportInvoiceData($resellerHandoverFeId, [114, 115, 116, 117], 'ADDON(R)');
    }

    private function exportInvoiceData($resellerHandoverFeId, array $productIds, string $type)
    {
        try {
            Log::info("Starting FE {$type} Sales Invoice Data export for Reseller Handover FE ID: " . $resellerHandoverFeId);

            // Decrypt the reseller handover FE ID
            $decryptedHandoverId = Encryptor::decrypt($resellerHandoverFeId);
            Log::info('Decrypted reseller handover FE ID: ' . $decryptedHandoverId);

            // Get the reseller handover FE
            $resellerHandover = ResellerHandoverFe::findOrFail($decryptedHandoverId);

            Log::info('Reseller handover FE found: ' . $resellerHandover->id);

            // Get timetec_proforma_invoice
            $invoiceNo = $resellerHandover->timetec_proforma_invoice;

            if (empty($invoiceNo)) {
                Log::warning('No timetec_proforma_invoice found for reseller handover FE ID: ' . $decryptedHandoverId);
            ...
            // DocNo - Use reseller handover fe_id
            $docNo = $resellerHandover->fe_id ?? '';
            ...
            $tempFile = tempnam(sys_get_temp_dir(), 'reseller_invoice_data_fe_export_');
            ...
            $handoverId = $resellerHandover->fe_id ?? str_pad($resellerHandover->id, 3, '0', STR_PAD_LEFT);
            $filename = "{$type}_SALES_DATA_{$handoverId}_" .
                        str_replace([' ', '/', '\\', '&'], '_', $subscriberName) . '_' .
                        date('Y-m-d') . '.xlsx';

            Log::info("About to send FE {$type} Sales Invoice Data Excel file: " . $filename);
            ...
            Log::error("FE {$type} Sales Invoice Data export error: " . $e->getMessage());
    ...
    private function mapSalesAgent($resellerHandover)
    {
        ...
        if ($resellerHandover->reseller_name) {
            ...
            Log::info('Checking reseller handover FE reseller_name: ' . $salespersonName);
        }
