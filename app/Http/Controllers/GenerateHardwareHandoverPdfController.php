...
class GenerateHardwareHandoverPdfController extends Controller
{
    public function __invoke(HardwareHandover $hardwareHandover)
    {
        try {
            // Load company logo
            ...
            $pdf = Pdf::setOptions([
                ...
                'defaultFont' => 'Arial'
            ])->loadView('pdf.hardware-handover', compact('hardwareHandover', 'path_img', 'path_img2', 'signature', 'lead', 'creator'));
    ...
    public function generateInBackground(HardwareHandover $hardwareHandover)
    {
        try {
            $image = file_get_contents(public_path('/img/logo-ttc.png'));
            $img_base_64 = base64_encode($image);
