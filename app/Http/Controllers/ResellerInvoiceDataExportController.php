...
class ResellerInvoiceDataExportController extends Controller
{
    public function exportRenewalSales($resellerHandoverId)
    {
        return $this->exportInvoiceData($resellerHandoverId, [108, 109, 110, 111], 'RENEWAL');
    }

    public function exportAddOnSales($resellerHandoverId)
    {
        return $this->exportInvoiceData($resellerHandoverId, [114, 115, 116, 117], 'ADDON(R)');
    }

    private function exportInvoiceData($resellerHandoverId, array $productIds, string $type)
    {
        try {
            Log::info("Starting {$type} Sales Invoice Data export for Reseller Handover ID: " . $resellerHandoverId);

            // Decrypt the reseller handover ID
            $decryptedHandoverId = Encryptor::decrypt($resellerHandoverId);
            Log::info('Decrypted reseller handover ID: ' . $decryptedHandoverId);

            // Get the reseller handover
            $resellerHandover = ResellerHandover::findOrFail($decryptedHandoverId);

            Log::info('Reseller handover found: ' . $resellerHandover->id);

            // Get timetec_proforma_invoice
            $invoiceNo = $resellerHandover->timetec_proforma_invoice;

            if (empty($invoiceNo)) {
                Log::warning('No timetec_proforma_invoice found for reseller handover ID: ' . $decryptedHandoverId);
            ...
            // DocNo - Use reseller handover fb_id or similar identifier
            $docNo = $resellerHandover->fb_id ?? '';
            ...
            // UDF fields
            $salesAdmin = $salesAgent; // Can be customized if needed
            $billingType = $type === 'RENEWAL' ? 'Renewal' : 'Renewal Addon';
            $cancelled = '';
            $udfSupport = auth()->user()->autocount_name ?? '';

            // Create header row to match InvoiceDataExportController
            $headers = [
                'DocNo',
                'DocDate',
                'CompanyName',
            ...
            $tempFile = tempnam(sys_get_temp_dir(), 'reseller_invoice_data_export_');
            ...
            $handoverId = $resellerHandover->fb_id ?? str_pad($resellerHandover->id, 3, '0', STR_PAD_LEFT);
            $filename = "{$type}_SALES_DATA_{$handoverId}_" .
                        str_replace([' ', '/', '\\', '&'], '_', $subscriberName) . '_' .
                        date('Y-m-d') . '.xlsx';

            Log::info("About to send {$type} Sales Invoice Data Excel file: " . $filename);
            ...
            Log::error("{$type} Sales Invoice Data export error: " . $e->getMessage());
    ...
    private function mapSalesAgent($resellerHandover)
    {
        ...
        // Try to get salesperson from reseller handover relationship or field
        if ($resellerHandover->salesperson) {
            $salespersonName = strtoupper(trim($resellerHandover->salesperson));
            Log::info('Checking reseller handover salesperson: ' . $salespersonName);
        }
