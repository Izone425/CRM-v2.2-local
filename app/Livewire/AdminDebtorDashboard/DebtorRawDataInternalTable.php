...
class DebtorRawDataInternalTable extends Component implements HasForms, HasTable
{
            ...
            ->query(
                DebtorAging::query()
                    ...
                    ->where(function ($query) {
                        $query->where('company_name', 'like', '%TIMETEC%')
                            ->orWhere('company_name', 'like', '%TimeTec%')
                            ->orWhere('company_name', 'like', '%Timetec%');
                    })
            )
            ->columns([
    ...
    {
        return view('livewire.admin-debtor-dashboard.debtor-raw-data-internal-table');
    }
}
