...
class DebtorRawDataOutstandingTable extends Component implements HasForms, HasTable
{
            ...
            ->query(
                DebtorAging::query()
                    ...
                    ->where(function ($query) {
                        $query->whereNot('debtor_code', 'like', '%STL%')
                            ->where(function ($innerQuery) {
                                $innerQuery->whereNot('company_name', 'like', '%TIMETEC%')
                                    ->whereNot('company_name', 'like', '%TimeTec%')
                                    ->whereNot('company_name', 'like', '%Timetec%');
                            });
                    })
            )
    ...
    {
        return view('livewire.admin-debtor-dashboard.debtor-raw-data-outstanding-table');
    }
}
