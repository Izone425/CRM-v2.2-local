                                                            ...
                                                            <div class="booking-cell actions-cell">
                                                                <div class="action-buttons">
                                                                    @if($booking->status === 'BOOKING')
                                                                        <button type="button" wire:click.stop="openApplyModal({{ $booking->id }})" class="action-btn apply-btn">Apply</button>
                                                                    @endif
                                                                    @if(auth()->user()->role_id == 3 && $booking->status === 'APPLY')
                                                                        <button type="button" wire:click.stop="openApproveModal({{ $booking->id }})" class="action-btn approve-btn">Approve</button>
                                                                    @endif
                                                                    @if($booking->submitted_by === auth()->user()->name || auth()->user()->role_id == 3)
                                                                        <button type="button" wire:click.stop="openCancelModal({{ $booking->id }})" class="action-btn cancel-btn">Cancel</button>
                                                                    @endif
                                                                </div>
                                                                <button class="expand-attendees-btn {{ $this->isBookingExpanded($booking->id) ? 'expanded' : '' }}" wire:click.stop="toggleBooking({{ $booking->id }})">
