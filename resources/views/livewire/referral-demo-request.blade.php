            ...
            <div class="card" style='width: 500px;'>
                <div class="card-body">
                    <form wire:submit.prevent="submit">
                        <h6 style="text-align: center;">Referral Details</h6>
                        ...
                        <div class="mb-3 form-group">
                            <label for="referral_email" class="form-label" style="font-weight:bold">Referral Email <span class="text-danger">*</span></label>
                            <input type="referral_email" id="referral_email" name="referral_email" class="form-control" wire:model.defer="referral_email" required>
                            @error('referral_email') <span class="text-danger">{{ $message }}</span> @enderror
                        </div>

                        <div class="mb-3 form-group">
                            <label for="referral_phone" class="form-label" style="font-weight:bold">Referral Phone <span class="text-danger">*</span></label><br>
                            <input class="form-control" style="width: 465px" id="referral_phone" type="tel" name="referral_phone"/>
                            <div id="referral-error-msg" style="color: red;"></div>
                                @error('referral_phone') <span style="color: red; font-size: 0.875rem; font-weight: bold;">{{ $message }}</span> @enderror
                        </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
    // Make sure the #phone input element exists before proceeding
    var input = document.querySelector("#referral_phone");
    if (!input) {
        console.error("Phone input field with ID 'phone' not found.");
        return;
    }
    ...
    var errorMsg = document.querySelector("#referral-error-msg"); // Ensure error element exists
    if (!errorMsg) {
        console.error("Error message element with ID 'error-msg' not found.");
    }
    ...
    function validateAndDispatch() {
            if (iti.isValidNumber()) {
                var referralPhoneNumber = iti.getNumber(); // Get the full phone number with the dial code
                // Dispatch the phone number to Livewire
                Livewire.dispatch('updateReferralPhone', [referralPhoneNumber]);
            }
        }
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        Livewire.on('refreshPage', () => {
            ...
            }, 0); // Immediately refresh after submission
        });
    });
</script>
