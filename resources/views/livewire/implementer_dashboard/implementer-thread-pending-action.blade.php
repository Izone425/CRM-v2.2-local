<div class="p-4 bg-white rounded-lg shadow-lg" style="height: auto;" x-data="{ showRefresh: false }" @mouseenter="showRefresh = true" @mouseleave="showRefresh = false">
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2">
            <h3 class="text-lg font-bold">Thread — Pending First Reply</h3>
            @php
                $filterLabels = [
                    'all' => 'All',
                    'overdue' => 'Overdue',
                ...
                $currentLabel = $filterLabels[$statusFilter] ?? 'All';
                $currentColor = $filterColors[$statusFilter] ?? 'bg-gray-100 text-gray-700';
            @endphp
            <span class="px-2 py-0.5 text-xs font-semibold rounded-full {{ $currentColor }}">
                Filter: {{ $currentLabel }}
            </span>
