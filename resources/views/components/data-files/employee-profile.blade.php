<style>
    ...
    .section-title {
        margin-bottom: 0.75rem;
        font-weight: 500;
        color: #3182ce; /* Blue for employee profile */
        font-size: 0.875rem;
    }
...
@php
    ...
    $userInfoFiles = collect($files)->where('subcategory', 'user_information');
@endphp

<div class="section-container">
    <h4 class="section-title">USER INFORMATION</h4>
    @if($userInfoFiles->count() > 0)
        <div class="files-grid">
            @foreach($userInfoFiles as $file)
                <div class="file-item">
                    <div class="file-name">
                        @php
                            $originalName = basename($file->filename);
                            $extension = pathinfo($originalName, PATHINFO_EXTENSION);
                            $shortName = "Employee Profile ";
                        @endphp
