<style>
    ...
    .section-title {
        margin-bottom: 0.75rem;
        font-weight: 500;
        color: #7e3af2; /* Purple for payroll module */
        font-size: 0.875rem;
    }
...
@php
    ...
    $employeeInfoFiles = collect($files)->where('subcategory', 'payroll_employee_info');
    $salaryDataFiles = collect($files)->where('subcategory', 'payroll_employee_salary_data');
    $accumulatedItemFiles = collect($files)->where('subcategory', 'payroll_accumulated_item_ea');
    $basicInfoFiles = collect($files)->where('subcategory', 'payroll_basic_information');
@endphp
