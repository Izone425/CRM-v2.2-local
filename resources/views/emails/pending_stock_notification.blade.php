...
<body>
    <div class="container">
        <div class="header">
            <h2>Hardware Handover: Pending Stock</h2>
        </div>
                                        ...
                                        @if($details['quantity'] !== 0)
                                            <tr>
                                                ...
                                                <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">
                                                    <span style="color: red;">Pending Stock</span>
                                                </td>
                                            </tr>
                                        @endif
                                    @endforeach
                    ...
                    <td>
                        @if(!empty($emailContent['invoiceFiles']) && count($emailContent['invoiceFiles']) > 0)
                            @foreach($emailContent['invoiceFiles'] as $index => $fileUrl)
                                <a href="{{ $fileUrl }}" target="_blank" class="button" style="background-color: #f0932b; color: white; padding: 5px 10px; text-decoration: none; border-radius: 4px; font-size: 12px; display: inline-block; margin-right: 5px; margin-bottom: 5px; width: 75px; text-align: center">
                                    Invoice {{ $index + 1 }}
                                </a>
                            @endforeach
                        @else
                    ...
                    <td>
                        @if(!empty($emailContent['salesOrderFiles']) && count($emailContent['salesOrderFiles']) > 0)
                            @foreach($emailContent['salesOrderFiles'] as $index => $fileUrl)
                                <a href="{{ $fileUrl }}" target="_blank" class="button" style="background-color: #f0932b; color: white; padding: 5px 10px; text-decoration: none; border-radius: 4px; font-size: 12px; display: inline-block; margin-right: 5px; margin-bottom: 5px;">
                                    Sales Order {{ $index + 1 }}
                                </a>
                            @endforeach
                        @else
